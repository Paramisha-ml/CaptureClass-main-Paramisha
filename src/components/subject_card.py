...
def subject_card(name, code, section, stats=None, footer_callback=None):
    html = f"""
        ...
        <h3 style="margin:0; color: rgba(0, 0, 0, 0.75); font-size: 1.5rem ">{name}</h3>
    ...
    if stats:
        ...
        for icon, label, value in stats:
            html+= f'<div style="background: #EB459E10; color: rgba(0, 0, 0, 0.75); padding:5px 12px; border-radius:12px; font-size:0.9rem">{icon} <b style="color: rgba(0, 0, 0, 0.75);">{value}</b> {label}</div>'
